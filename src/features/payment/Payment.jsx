...
import { orderPaymentIdAsync } from "../order/orderSlice";
...
export default function Payment() {
  ...
  const cardPaymentOrders = orders?.filter(
    (order) => order.paymentMethod === "card"
  );

const handlePaymentId=(e,id)=>{
dispatch(orderPaymentIdAsync(id));
}

  return (
    <div>
      {cardPaymentOrders &&
        cardPaymentOrders.map((order) => (
          <div
            key={order.id}
            className="mx-auto max-w-7xl px-4 mt-12  bg-white py-6 sm:px-6 lg:px-8"
          >
            <div className="border-t border-gray-200 px-4 py-6">
              <div className="flex justify-between sm:flex-wrap">
                ...
                <button
                  type="button"
                  onClick={e=>handlePaymentId(e,order.id)}
